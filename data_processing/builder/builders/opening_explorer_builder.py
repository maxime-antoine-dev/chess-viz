...
class OpeningExplorerBuilder(BaseBuilder):
    ...
    def _build_recursive(self, df: pl.DataFrame, depth: int) -> List[Dict]:
        ...
        if df_at_depth.is_empty():
            return []
        stats = (
            df_at_depth.group_by(["current_m", "clean_opening"])
            .agg([
                pl.len().alias("c"),
        ...
        for row in stats.to_dicts():
            ...
            node = {
                "move": row["current_m"],      # move
                "name": row["clean_opening"],  # name
                "count": row["c"],              # count
                "stats": [row["w"], row["d_rate"], row["b"]] # stats [white, draw, black]
            }

            children = self._build_recursive(sub_df, depth + 1)
            if children:
                node["children"] = children # descendants
